...
int insertOnTreeAVL(TREE** adrs, TREE* root, int id);
...
int insertOnTreeAVL(TREE** adrs, TREE* root, int id)
{
    ...
    {   
        if (id > root->id)
        {
            if (NULL != root->right)
            {
                insertOnTreeAVL(adrs, root->right, id);
            }else
            {
                insertOnRightSizeTree(root, id);
                verifyDirectionalAVLTree(adrs, *adrs, id);
            }
        }else if(id < root->id)
        {
            if (NULL != root->left)
            {
                insertOnTreeAVL(adrs, root->left, id);
            }else
            {
